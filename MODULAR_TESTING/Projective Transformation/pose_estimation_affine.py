...
original_points = np.float32([[0, new_height], [new_width, new_height], [new_width, 0]])
# Scaled points
scaled_points = original_points * scale

# Define points for affine transformation
pts1 = np.float32([[115,351], [633,438], [901,170]])
pts2 = scaled_points
...
for idx, result in enumerate(results):
    ...
    for box in boxes:
        ...
        if box.cls == 0:  # Class ID 0 is for 'person' in COCO dataset
            ...
            # Apply affine transformation to the head center
            transformed_head_center = cv2.transform(np.array([head_center]), M)[0][0]
