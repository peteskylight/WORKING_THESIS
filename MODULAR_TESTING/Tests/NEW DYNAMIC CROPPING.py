...
# Load YOLOv8 model (supports detection, segmentation, and tracking)
model = YOLO('yolov8x.pt')  # Segmentation-enabled model


# '''
#     VIDEO_PATH = r"C:\Users\Bennett\Documents\WORKING_THESIS\RESOURCES\1ST SURVEY\PROCESSED\Front 3 persons.mp4"  
...
MASK_HEIGHT = int(frame_height * (1/18))  # Mask covering the top 2/12th of height
MASK_COLOR = (0, 0, 255)  # Red mask for visualization

# Track ID Mapping
id_map = {}  # Maps original YOLO IDs to sequential IDs
next_id = 1  # Start counting from 1

def create_roi_mask(frame, mask_height):
    """Create a transparent red overlay for the masked area (TOP part of the frame)."""
    mask_overlay = frame.copy()
    cv2.rectangle(mask_overlay, (0, 0), (frame.shape[1], mask_height), MASK_COLOR, -1)
...
while cap.isOpened():
    ...
    masked_frame = create_roi_mask(frame, MASK_HEIGHT)

    # Apply YOLO detection with tracking
    results = model.track(frame, persist=True, conf=0.5, classes = 0, iou = 0.3)
    ...
    for result in results:
        ...
        for box, track_id, class_id in zip(boxes, track_ids, class_ids):
            ...
            # Ignore objects that are NOT humans (assuming class_id=0 is human)
            if int(class_id) != 0:  
                continue  # Ignore chairs, desks, etc.

            # Ignore persons inside the masked area
            if y1 <= MASK_HEIGHT:
                continue  
